...
<head>
    ...
    if (parseInt(value) === 1 && index !== sourceId) {
        CloseMenuItem(index);
    }
});
        ...
        function MouseOverSubmenuItem(source) {
            if (source.style) {
                source.style.backgroundColor = "#4F4F4F";
            }
        }

        function MouseOutSubmenuItem(source) {
            if (source.style) {
                source.style.backgroundColor = "#1C1C1C";
            }
        }
    </script>
    <?= template_place_holder('head_end'); ?>
	<style>
        /* Sobreposição que bloqueia a interação com o resto do site */
        .overlay {
            position: fixed;
            top: 0;
            left: 0;
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (window.location.search.includes('mpnotification')) {
                document.body.classList.add('overlay-active');
            }
        });
    </script>
                            ...
                            <div class="BorderTitleText"
                                ...
                                <div class="InfoBar">
                                  <!--  <img class="InfoBarBigLogo"
									...
									<?php } ?>										
                                    <?php if (!empty($config['cast_link'])) { ?>
                                     <img class="InfoBarBigLogo"
                                          src="<?= $template_path; ?>/images/cast/icon-cast.png"
                                          width="13"
                                          style="margin-left: 8px; position: relative; top: -3px;">
                                    <span class="InfoBarNumbers">
                                       <a class="InfoBarLinks" href="<?= $config['cast_link']; ?>">
                                       <span class="InfoBarSmallElement">
    Live Casts (<?= $total_casts ?>)
</span>
